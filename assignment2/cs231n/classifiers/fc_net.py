...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dx2, grads['W2'], grads['b2'] = affine_relu_backward(softmax_grad, cache2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        i += 1
        self.params[f'W{i+1}'] = weight_scale * np.random.randn(dims[i], num_classes)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            out, affine_cache[i] = affine_forward(out, 
                                               self.params[f'W{i+1}'], 
                                               self.params[f'b{i+1}'])
            if self.use_batchnorm:
                out, bn_cache[i] = batchnorm_forward(out, 
        ...
        i += 1
        scores, affine_cache[i] = affine_forward(out, 
                                               self.params[f'W{i+1}'], 
                                               self.params[f'b{i+1}'])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ...
        dout, grads[f'W{i+1}'], grads[f'b{i+1}'] = affine_backward(dout, affine_cache[i])
        ...
        for i in range(i, -1, -1):
            ...
            if self.use_batchnorm:
                dout, grads[f'gamma{i+1}'], grads[f'beta{i+1}'] = batchnorm_backward(dout, bn_cache[i])
            dout, grads[f'W{i+1}'], grads[f'b{i+1}'] = affine_backward(dout, affine_cache[i])
            grads[f'W{i+1}'] += self.reg * self.params[f'W{i+1}']
        ############################################################################
        #                             END OF YOUR CODE                             #
